...
- When GitHub cannot submit a formal review because the independent reviewer shares the PR author's account, the configured reviewer automation may use a top-level terminal marker. A marker is an authoritative candidate only when GitHub API metadata records `performed_via_github_app.slug = chatgpt-codex-connector`; missing/wrong-App markers are ignored before selecting the latest reviewer decision. The exact PR, head, policy, and decision contract must still validate. `user.login` and `author_association` do not grant or deny authority. This repository-local App provenance requirement is necessary but does not waive the independent-lineage eligibility gate above. The dedicated check-publisher App is a separate output identity and cannot supply reviewer provenance. **Implementation agents and their delegated descendants must never use the reviewer automation or emit `aw-chatgpt-review` markers for that implementation lineage.**
