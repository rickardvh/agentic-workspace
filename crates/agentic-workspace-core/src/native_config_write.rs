...
pub(crate) fn contract() -> Result<Value, CoreError> {
    ...
    alternatives.push(json!({"properties":{"source":{"enum":crate::native_payload::paths()},"key":{"const":PAYLOAD_KEY},"value":{"type":"string"}}}));
    ...
    let recovery = json!({"$schema":"https://json-schema.org/draft/2020-12/schema","type":"object","properties":{"source":{"enum":([vec![SHARED,LOCAL],crate::native_payload::paths()].concat())},"record_revision":{"type":"string"}},"required":["source","record_revision"],"additionalProperties":false});
