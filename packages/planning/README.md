...
### Proportional owner records

New live execplans use a compact authoritative kernel. Its universal top-level fields are `kind`, `id`, `title`, `owner_level`, `lifecycle`, `phase`, `revision`, `intent`, `parent`, `scope`, `relationships`, `references`, `next_action`, `blockers`, `proof`, `continuation`, and `specialist_contracts`. Lifecycle and phase are closed sets; unknown values never make a record active.

Workflow-specific state is optional and typed. Delegation, assurance, review, decomposition, and prep-only concerns are referenced through `specialist_contracts` only when that workflow applies. Lane, decomposition, and review records remain their own specialist owner families rather than being copied into every execplan.

Current live legacy execplans are migrated by the supported upgrade writer. Historical archives remain readable in their legacy shape and are not bulk-loaded or rewritten. Ordinary closeout removes the completed live owner and retains a compact, schema-backed tombstone under `closeout-evidence/`; full archives require an explicit retention choice. Report audit traversal is cursor-paginated and loads only the requested page.

The representative scaffold benchmark fell from 11,658 bytes and 38 top-level fields to 1,256 bytes and 17 fields (about 89% fewer bytes and 55% fewer top-level fields). Guardrails cap a live execplan at 6 KB and compact closeout evidence at 3 KB.
...
Selected active-owner fields (`planning_record`, `active_contract`, `resumable_contract`, and `continuation_view`) use a declared dependency plan and read only Planning state plus the selected owner. Their `selection_cost` diagnostics report the resolver, cache identity, exact invalidation provenance, and historical sources omitted. Default summary likewise keeps review and archive history off its execution path. Historical closeout inspection is explicit and paginated through `agentic-workspace planning report --verbose --audit-page-size <n> --audit-cursor <cursor> --format json`. Maintained budgets are a 4 KiB quiet default packet, a 2 second median clean-process selected-owner query, and no more than 20% median latency growth with 1,000 historical closeouts; regression failures must identify the selected resolver or field.
