...
- Keep the default friction-response order explicit: adapt inside the workspace first when that is the honest cheap fix, and only then promote repo-directed improvement when the root problem is genuinely external. See [`docs/signal-hygiene-contract.md`](signal-hygiene-contract.md) for the authoritative adaptive discipline.
