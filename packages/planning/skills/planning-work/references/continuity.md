# Create or tighten accepted custody

Use current `planning.creation_requests` or the selected owner's update request.
State the concrete outcome, acceptance evidence, negative requirements and next
action using the returned fields. Preserve existing relationships; do not create
a second plan, copy a legacy template into a custom schema or hand-edit owner state.
