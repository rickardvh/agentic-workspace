...
`command_generation.load_command_package_ir(ir_path, schema_path)` loads and validates an explicit IR file against an explicit schema file. Host repositories provide those paths from their own integration wrappers; this package does not import Agentic Workspace contract tooling.
