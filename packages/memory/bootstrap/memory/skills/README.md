...
These shipped shared skills are installed under `.agentic-workspace/memory/skills/`. When a repository needs a local memory workflow beyond these, add a repo-owned sibling skill under `memory/skills/` instead of editing the shared core skills unless the reusable shared procedure itself changed.
