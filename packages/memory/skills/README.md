...
If the bundled skill is not already visible, prefer the installed `agentic-memory` command or the repo's checked-in `.agentic-workspace/memory/UPGRADE-SOURCE.toml` as the source of truth for any remote `uvx` or `pipx` runner spec.
