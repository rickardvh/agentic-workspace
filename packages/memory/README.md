...
- **Pull/capture decision packet support.** Workspace `start`, `implement`, and closeout report surfaces can project a `memory_decision_packet` that asks the agent to explicitly pull, dismiss, capture, route elsewhere, or leave unwritten durable knowledge. `memory route` accepts structured `--stage`, `--files`, and `--surface` inputs; `--task` is context, not matching authority. `memory capture-note` returns Memory candidates and non-Memory owner routes so "no Memory write" can still be an explicit, successful decision.
