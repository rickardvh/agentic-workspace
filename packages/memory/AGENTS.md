...
- authority: adapter
- canonical_source: `packages/memory/src/` plus root `AGENTS.md`
- safe_to_edit: true
...
- This package contains the reusable `agentic-memory` source, shipped payload, package-local skills, and tests.
- Treat `bootstrap/` as packaged payload content for target repositories, not as the active memory system of this monorepo.
- Treat the package-managed memory home under `.agentic-workspace/memory/` and the installed memory projection under `.agentic-workspace/memory/repo/` as the operational authority surfaces for this repository.
...
- Installed layer: the target repo's `.agentic-workspace/memory/repo/` tree and `.agentic-workspace/memory/` surfaces are the live memory contract after bootstrap.
- In this monorepo, the package-managed memory home under `.agentic-workspace/` is the active design space for shared memory machinery, while the repo-facing `.agentic-workspace/memory/repo/` tree is the installed contract; this package directory is not a second active memory install.
...
- Do not recreate package-local operational installs in this monorepo; keep the package-managed `.agentic-workspace/` home authoritative and treat the repo-facing `.agentic-workspace/memory/repo/` tree as the installed contract.
