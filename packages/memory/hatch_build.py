...
class CustomBuildHook(BuildHookInterface):
    def initialize(self, version: str, build_data: dict[str, object]) -> None:
        if self.target_name != "wheel":
            return
        root = Path(self.root)
        embedded_generated_root = root / "src" / "repo_memory_bootstrap" / "_generated_cli_package_impl"
        ...
        for source, destination in list(force_include.items()):
            ...
            if destination_path.parts[:3] != ("src", "repo_memory_bootstrap", "_generated_cli_package_impl"):
                continue
