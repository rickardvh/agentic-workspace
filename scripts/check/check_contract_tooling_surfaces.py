...
def _validate_operation_registry(payload: dict[str, object]) -> list[str]:
    ...
    python_adapter = (REPO_ROOT / "src/agentic_workspace/result_adapter.py").read_text(encoding="utf-8")
    python_binding = (REPO_ROOT / "packages/memory/src/repo_memory_bootstrap/_installer_shared.py").read_text(encoding="utf-8")
    ...
    if "from repo_memory_bootstrap._installer_shared import mutation_outcome_from_actions" not in python_adapter:
        errors.append("workspace Python result adapter must reuse the direct-module mutation outcome binding")
