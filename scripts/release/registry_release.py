    ...
    with tempfile.TemporaryDirectory(prefix="aw-registry-consumer-") as directory:
        ...
        subprocess.run(
            [
                ...
                "--input-type=module",
                "-e",
                "import {start,invoke} from '@agentic-workspace/workspace-cli'; if(start({target:'.'}).decision_packet.status!=='direct') throw Error('native start failed'); let rejected=false; try {invoke({target:'.',invocation:{}})} catch {rejected=true} if(!rejected) throw Error('invalid invocation accepted');",
            ],
            cwd=root,
            env=env,
            check=True,
