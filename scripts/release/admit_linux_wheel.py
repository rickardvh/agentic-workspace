...
def admit(directory):
    ...
    wheels = list(directory.glob(f"agentic_workspace-*-linux_{architecture}.whl"))
    if len(wheels) != 1:
    ...
    match = re.search(r'following platform tag:\s*"(manylinux_2_([0-9]+)_' + architecture + ')"', audit)
    ...
    subprocess.run(
        ["uvx", "--from", "wheel==0.45.1", "wheel", "tags", "--platform-tag", f"manylinux_2_39_{architecture}", "--remove", str(wheel)],
        check=True,
    )
