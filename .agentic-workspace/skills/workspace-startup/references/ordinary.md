...
Existing users may invoke AW/setup manually once. Ordinary adoption/refresh
establishes the stable entry in the existing managed boundary. Later compatible
updates need no reminder. Installation only supplies executable bytes; no package
manager, interpreter or shell hook participates. Missing runtime remains unknown.
Use managed refresh for changed package bytes and setup assessment only when due.
Unchanged deferred choices remain queryable; resume only for a changed prerequisite,
dependent work or an explicit revisit.
