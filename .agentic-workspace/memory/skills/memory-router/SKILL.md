...
3. Use the repo's routing help first:
   - run `agentic-workspace memory route --files <paths...>` when file paths are known
   - run `agentic-workspace memory route --surface <surface...>` when the work is easier to describe by surface
