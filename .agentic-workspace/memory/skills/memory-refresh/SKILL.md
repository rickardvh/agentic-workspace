...
3. Use the repo's routing help first:
   - run `agentic-workspace memory sync-memory --files <paths...>` when available
   - run `agentic-workspace memory route --files <paths...>` when useful for note selection
...
11. If a note is acting as an improvement signal, run `agentic-workspace memory promotion-report --notes <note>` and prefer the smallest justified post-remediation memory shape: keep the note only if it still saves rediscovery cost, otherwise shrink it to a stub or remove it.
