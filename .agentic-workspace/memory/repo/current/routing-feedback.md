...
- 2026-05-22: `planning close-item` / `archive-plan` still made a completed execplan closeout feel manual when the retained archive exceeded the structured-file inventory guardrail. PR #1099 addressed the `close-item` cleanup path by skipping oversized retained archive evidence and completing after distillation; keep watching for explicit `archive-plan --retain-archive` cases where clearer rerun guidance may still matter.
