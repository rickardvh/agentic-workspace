...
- 2026-05-22: `planning close-item` / `archive-plan` still made a completed execplan closeout feel manual when the retained archive exceeded the structured-file inventory guardrail. The useful improvement is one closeout path that auto-distills overlarge plans or reports the exact next command/state mutation needed, instead of requiring the agent to shrink the plan and retry.
