...
- Prefer package commands for planning mutations when available: `agentic-planning new-plan --id <id> --title <title> --activate`, `agentic-planning promote-to-plan <item-id>`, and `agentic-planning archive-plan <plan>`. `agentic-planning-bootstrap` remains a compatibility alias.
