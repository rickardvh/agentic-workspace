...
1. Before implementation, run `agentic-workspace preflight --task "<task>" --format json` using the user's request as `<task>` for takeover, recovery, first-contact context, and task-specific skill routing; omit `--task` only when the task text is unavailable. Use `primary_next_action`, `skill_routing`, and `vague_outcome_orientation` when present before opening raw `.agentic-workspace` files.
...
4. If the configured startup file, posture, or workflow obligations matter, run `agentic-workspace config --target . --profile compact --format json`.
5. If you need the ordinary startup order, run `agentic-workspace defaults --section startup --format json`.
6. If the request asks for repo-visible durable state, handoff, continuation, or a plan for later agents, run `agentic-workspace planning --format json` and use its `durable_state_bridge` route before creating files.
7. If the user asks you to prepare broad work for later continuation and does not ask you to implement, create or continue canonical checked-in Planning state now, verify it with summary, then stop. A proposal-only answer is not durable handoff.
8. If module ownership or the abstraction ladder is unclear, read `.agentic-workspace/docs/module-map.md`.
9. Open raw planning, memory, module, or workflow files only when compact output points there.
