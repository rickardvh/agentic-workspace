...
1. Before implementation, run `agentic-workspace preflight --task "<task>" --format json` using the user's request as `<task>` for takeover, recovery, first-contact context, and task-specific skill routing; omit `--task` only when the task text is unavailable.
