...
| Workspace policy | Repo-owned default preset, improvement latitude, optimization bias, advanced-feature opt-ins, and update intent. | `.agentic-workspace/config.toml` | `agentic-workspace config --target ./repo --profile compact --format json` |
| Local invocation and integration aids | Optional machine-local CLI invocation and vendor/runtime helpers that make local compliance cheaper without becoming shared truth. | `.agentic-workspace/config.local.toml` plus `.agentic-workspace/local/integrations/` | `agentic-workspace config --target ./repo --profile compact --format json` |
...
`.agentic-workspace/config.toml` is repo-owned policy. Its installed header says agents may edit it directly only when changing repo policy, and should use `agentic-workspace config --target . --profile compact --format json` to inspect the resolved effective view before relying on a setting.
...
2. `agentic-workspace config --target ./repo --profile compact --format json`
...
| --- | --- | --- |
| Startup path | `agentic-workspace defaults --section startup --format json` | `agentic-workspace config --target ./repo --profile compact --format json`, then `AGENTS.md` |
