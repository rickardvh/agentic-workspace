...
- Keep capture separate from promotion into `roadmap` or `todo.active_items` in `.agentic-workspace/planning/state.toml`.
- Preserve the higher trust of friction-derived improvement signals.
- Avoid speculative queue churn from unconfirmed analysis.

## Ownership

- `.agentic-workspace/planning/reviews/` owns bounded review artifacts.
- `roadmap` in `.agentic-workspace/planning/state.toml` owns inactive future candidates once a finding is promoted.
- `todo.active_items` in `.agentic-workspace/planning/state.toml` plus `.agentic-workspace/planning/execplans/` own active planned work once promotion becomes immediate execution.
...
| --- | --- | --- | --- | --- | --- |
| `contract-integrity` | Check whether repo claims, docs, and enforced surfaces still agree. | Canonical docs, check wrappers, claimed contract files | broken references, docs-code drift, promise-vs-enforcement gaps | `.agentic-workspace/planning/state.toml` (`roadmap`) or canonical docs | 3 findings |
| `planning-surface` | Check whether `todo.active_items`, `roadmap`, execplans, and planning docs still form a coherent execution contract. | `.agentic-workspace/planning/state.toml`, active execplans, planning checks | stale queue state, missing execplan links, contradictory sequencing guidance | `roadmap` or `todo.active_items` in `.agentic-workspace/planning/state.toml` | 3 findings |
| `current-context` | Check whether current-state notes still behave like weak-authority re-orientation rather than shadow planning. | `.agentic-workspace/memory/repo/current/`, shared workflow docs, active planning surface | planner-like drift, authority ambiguity, restart-cost inflation | `.agentic-workspace/planning/state.toml` (`roadmap`), canonical docs, or memory | 2 findings |
| `memory-boundary` | Check whether memory stays routed, justified, and subordinate to canonical docs and planning. | `.agentic-workspace/memory/repo/index.md`, memory workflow docs, representative notes, freshness output | over-capture, duplicated authority, missing promotion to canonical docs, noisy routing | `.agentic-workspace/planning/state.toml` (`roadmap`), canonical docs, or memory | 3 findings |
| `maintainer-workflow` | Check whether maintainer-facing commands and docs still describe a real, runnable workflow. | maintainer docs, check wrappers, lifecycle commands, package/root guidance | broken maintainer paths, missing docs, overclaimed checks, boundary confusion | `.agentic-workspace/planning/state.toml` (`roadmap`) or canonical docs | 3 findings |
| `source-payload-install` | Check whether package source, shipped payload, and root operational install still line up. | package sources, bootstrap payload, root installed surfaces, lifecycle tests | source/payload drift, install mismatch, upgrade ambiguity, generated-root confusion | `.agentic-workspace/planning/state.toml` (`roadmap` or `todo.active_items`) | 3 findings |
| `generated-surface-trust` | Check whether generated helper surfaces still faithfully reflect their canonical source. | manifest sources, generated docs, render scripts, installer checks | stale generated docs, missing render coverage, misleading derived guidance | `.agentic-workspace/planning/state.toml` (`roadmap`) or canonical docs | 2 findings |
| `skill-system` | Check whether skill discovery, naming, routing, and arsenal boundaries still make skills cheaper to use than generic reasoning for stable workflows. | skill registries, skill catalog docs, recommendation output, skill placement/ownership surfaces | registry drift, misleading skill docs, routing misses, muddled skill ownership, missing or badly placed repeatable workflows | `.agentic-workspace/planning/state.toml` (`roadmap`), canonical docs, or skills | 3 findings |
| `doctrine-refresh` | Check whether long-horizon doctrine still matches current dogfooding reality without turning into stale narrative or hidden backlog. | doctrine pages, maturity framing, ecosystem stance, recent archived execplans and review outcomes | stale direction, conflicting doctrine, hidden queue residue, outdated maturity claims | canonical docs or `.agentic-workspace/planning/state.toml` (`roadmap`) | 2 findings |
| `validation-lane` | Check whether the documented validation lane still proves the promised contract. | package tests, repo checks, maintainer command docs, failing gaps | missing regression coverage, weak check aggregation, undocumented required steps | `.agentic-workspace/planning/state.toml` (`roadmap` or `todo.active_items`) | 3 findings |
| `context-cost` | Check whether startup and handoff surfaces stay cheap enough for routine agent use. | startup docs, agent manifest, memory routing surfaces, review/read requirements | over-reading, oversized startup bundles, repeated low-signal reads | `.agentic-workspace/planning/state.toml` (`roadmap`), memory, or canonical docs | 2 findings |
| `review-promotion` | Check whether review findings are being promoted, deferred, or deleted with discipline. | `.agentic-workspace/planning/reviews/`, roadmap intake, archived plans, status footers | stale review residue, duplicate candidates, weak promotion hygiene | `.agentic-workspace/planning/state.toml` (`roadmap`) or review cleanup | 2 findings |
...
2. promote it to `roadmap` in `.agentic-workspace/planning/state.toml` when the work is plausible future value but not active now
3. promote it to `todo.active_items` plus an execplan only when explicit maintainer choice, repeated friction, or clear urgency justifies activation
...
3. choose the routing surface:
   ...
   - `roadmap` in `.agentic-workspace/planning/state.toml` when the remediation is plausible future work
   - `todo.active_items` plus execplan when the remediation is explicitly active now
...
- Do not leave top-level `.agentic-workspace/planning/reviews/` full of old speculative notes.
