...
- Used the configured local target profile inventory in `.agentic-workspace/config.local.toml`.
...
- Keeping the outcome artifact separate from `.agentic-workspace/config.local.toml` was the quieter shape; the editable profile stays readable while the evidence log can grow independently.
