...
- Activation trigger: current migration work reaches the next boundary-cleanup tranche, at which point root `.agentic-workspace/memory/repo/`, leaked config, and hybrid package-owned `docs/` material should be collapsed back into package-owned homes unless they are strongly justified as true repo contracts.
...
- Discovered implications: repo-specific does not equal repo-owned; root `.agentic-workspace/memory/repo/`, root config, and hybrid `docs/` surfaces remain suspicious until the migration tranche explicitly proves otherwise or moves them inward.
...
- Proof achieved now: `agentic-workspace report` no longer crashes on empty findings, and the previous `.agentic-workspace/memory/repo/current/active-decisions.md` note was removed from the live memory contract.
...
- Next likely slice: reopen `#231` as inward boundary cleanup after the current migration tranche finishes, starting with explicit relocation targets for root `.agentic-workspace/memory/repo/`, leaked config, and package-owned planning/memory/install/routing material in `docs/`.
...
- Is repo-root `.agentic-workspace/memory/repo/` still a deliberate repo-owned contract, or has enough of it become package-managed that it should collapse back into `.agentic-workspace/`?
...
- Escalate when: resolving the remaining ambiguity now requires a product-level decision about whether `.agentic-workspace/memory/repo/`, `docs/`, or config are repo contracts by doctrine rather than by current implementation convenience.
...
- Core boundary docs: `.agentic-workspace/docs/ownership-authority-contract.md`, `.agentic-workspace/docs/compatibility-policy.md`, `docs/collaboration-safety.md`, `docs/integration-contract.md`, `docs/contributor-playbook.md`, `docs/architecture.md`
- Repo startup and config surfaces: `AGENTS.md`, `.agentic-workspace/config.toml`, `.agentic-workspace/WORKFLOW.md`, `.agentic-workspace/OWNERSHIP.toml`
- Repo docs and intake policy: `.agentic-workspace/planning/upstream-task-intake.md`, `.agentic-workspace/docs/installer-behavior.md`, `.agentic-workspace/docs/extraction-and-discovery-contract.md`
- Memory decision surfaces: `.agentic-workspace/memory/repo/decisions/installed-system-consolidation-2026-04-05.md`, `.agentic-workspace/memory/repo/decisions/workspace-orchestrator-ownership-ledger-2026-04-05.md`
...
- Memory payload surfaces: `packages/memory/bootstrap/.agentic-workspace/memory/repo/index.md`, `packages/memory/bootstrap/memory/system/WORKFLOW.md`, `packages/memory/bootstrap/memory/system/SKILLS.md`, `packages/memory/bootstrap/memory/bootstrap/README.md`, `packages/memory/bootstrap/memory/bootstrap/skills/install/SKILL.md`, `packages/memory/bootstrap/.agentic-workspace/memory/repo/skills/REGISTRY.json`, `packages/memory/.agentic-workspace/memory/repo/index.md`, `packages/memory/tests/fixtures/routing/memory-index-template.md`
- Planning mirrors: `packages/planning/bootstrap/.agentic-workspace/planning/upstream-task-intake.md`, `packages/planning/bootstrap/.agentic-workspace/docs/candidate-lanes-contract.md`
...
- Outcome delivered: fixed the empty-findings crash in `agentic-workspace report`, removed `.agentic-workspace/memory/repo/current/active-decisions.md` from the live memory contract, and exposed that the broader `#231` lane was closed on the wrong default.
...
- Knowledge promoted (Memory/Docs/Config): `.agentic-workspace/memory/repo/index.md`, `.agentic-workspace/memory/repo/manifest.toml`, `.agentic-workspace/memory/repo/decisions/README.md`, `src/agentic_workspace/cli.py`
...
- Proof achieved now: the bounded fixes in this slice are real, but they do not prove that root `.agentic-workspace/memory/repo/`, leaked config, or hybrid package-owned `docs/` material are justified repo-owned contracts.
...
- 2026-04-21: Prematurely closed the broader lane by treating root `.agentic-workspace/memory/repo/` and root config as justified repo-owned contracts; this archive now records that mistake and routes the larger intent back into roadmap planning with the corrected inward-default interpretation.
