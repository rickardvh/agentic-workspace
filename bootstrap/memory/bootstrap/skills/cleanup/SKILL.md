...
1. Confirm there is no remaining bootstrap work that still depends on `.agentic-memory/bootstrap/`.
2. Point out any last manual-review items that must be handled before cleanup.
3. Remove `.agentic-memory/bootstrap/`.
...
- Do not remove `.agentic-memory/skills/`, repo-owned `memory/skills/`, or any other durable memory content.
- Stop and report if cleanup would discard unresolved bootstrap work.

## Typical outputs

- confirmation that bootstrap work is complete
- removal of `.agentic-memory/bootstrap/`
