...
def render_quickstart(manifest: dict) -> str:
    ...
    lines.append("- Machine-readable routing: `tools/agent-manifest.json`")
    lines.append("")
    ...
    lines.append(f"- Shared memory workflow skills: `{skills.get('memory_source_dir', '.agentic-memory/skills')}`")
...
def main() -> int:
    QUICKSTART_PATH.write_text(render_quickstart(load_manifest()), encoding="utf-8")
    return 0
