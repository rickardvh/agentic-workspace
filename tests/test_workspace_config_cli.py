...
def test_config_command_layers_shared_local_config_below_repo_local_override(tmp_path: Path, capsys) -> None:
    ...
    shared.write_text(
        "schema_version = 1\n\n"
        "[workspace]\n"
        'cli_invoke = "python -c \\"import sys; from agentic_command_generation.workspace_generated_cli_package import main; raise SystemExit(main(sys.argv[1:]))\\""\n\n'
        "[runtime]\n"
    ...
    assert payload["workspace"]["cli_invoke"] == (
        'python -c "import sys; from agentic_command_generation.workspace_generated_cli_package import main; raise SystemExit(main(sys.argv[1:]))"'
    )
