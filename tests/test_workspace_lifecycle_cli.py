...
def test_upgrade_local_only_full_install_honors_configured_agent_instructions_file(tmp_path: Path, capsys) -> None:
    ...
    assert (repo_root / "GEMINI.md").read_text(encoding="utf-8") == "Follow instructions in `AGENTS.local.md` if present.\n"
    assert payload["config"]["workspace"]["agent_instructions_file"] == "GEMINI.md"
    assert "<!-- agentic-workspace:workflow:start -->" in (repo_root / "AGENTS.local.md").read_text(encoding="utf-8")
    startup_review_actions = [
        action
        for report in payload["reports"]
        for action in report["actions"]
        if action.get("path") in {"AGENTS.md", "GEMINI.md"} and "apply-local-entrypoint" in action.get("detail", "")
    ]
...
def test_install_real_init_creates_combined_memory_and_planning_surfaces(tmp_path: Path) -> None:
    ...
    ownership_text = (target / ".agentic-workspace" / "OWNERSHIP.toml").read_text(encoding="utf-8")
    assert "subsystems = []" in ownership_text
    assert 'paths = [".agentic-workspace/planning/**", "packages/planning/**"]' not in ownership_text
    assert 'id = "workspace-cli-runtime"' not in ownership_text
    assert 'id = "model-cli-harness"' not in ownership_text
    ...
    assert "Ordinary route:" in agents_text
    assert "before non-trivial answers" in agents_text
...
def test_install_real_init_can_use_gemini_as_root_startup_entrypoint(tmp_path: Path) -> None:
    ...
    assert "Ordinary route:" in gemini_text
    assert "before non-trivial answers" in gemini_text
...
def test_upgrade_preserves_repo_owned_agents_content_outside_workspace_fence(tmp_path: Path, capsys) -> None:
    ...
    assert "Ordinary route:" in updated
    assert "before non-trivial answers" in updated
