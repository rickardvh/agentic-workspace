...
def test_non_watching_poll_reports_single_poll_exact_head_binding(tmp_path: Path, monkeypatch, capsys) -> None:
    runner = FakeRunner(tmp_path)
    ...
    assert payload["mode"] == "single-poll"
...
def test_local_recheck_command_is_not_registered() -> None:
    parser = loop._parser()

    assert "recheck" not in parser.format_help()
    with pytest.raises(SystemExit):
        parser.parse_args(["recheck", "--pr", "12", "--head", HEAD_A])
