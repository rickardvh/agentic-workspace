...
- Selective-adoption contract clarity: decide whether the workspace layer is full-preset-only or a root-CLI contract appearing for any root-managed install, then add real tests for `--preset planning`, `--preset memory`, and explicit `--modules` cases to prevent future accidental contract drift. Promote when selective-adoption behavior is next touched or clarified. (mixed, high-confidence)
