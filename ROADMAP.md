...
- The install/adopt fixture-hardening tranche landed across planning installer fixtures and workspace CLI fixtures for docs-heavy repos, partial managed state, and mixed module ownership, so conservative adoption behavior is now exercised against more realistic ambiguity instead of only happy-path payload checks.
- Dogfooding that tranche surfaced a concrete lesson now folded back into plan history: adoption trust depends on proving both package-local ownership boundaries and workspace-level ambiguity reporting against the same repo shapes.
- The stable package contract freeze tranche landed across explicit contract shortlists, runtime doctor and verify-payload reporting, root workspace passthrough proof, adapter-path normalization, and a root maintainer note about contract inspection.
- Dogfooding that tranche surfaced a concrete lesson now folded back into plan history: package-level contract visibility is enough when the root lifecycle preserves module reports faithfully instead of inventing a second top-level schema.
- The first-party module contract tranche landed across explicit descriptor metadata for lifecycle arguments, install signals, workflow-overlap paths, generated-artifact classification, and centralized descriptor construction, so the next orchestration gap is registry-style discoverability rather than hidden contract shape.
- Dogfooding that tranche surfaced a concrete lesson now folded back into plan history: first-party module contract work gets materially safer once path ownership and lifecycle argument support live in the same descriptor model instead of partly in root heuristics and partly in package wiring.
