...
- Remaining: Issue #5, tighten `memory/current/` so it cannot be mistaken for active planning authority. The repo has already landed major contract work here, but the upstream issue is still open and is malformed because it also bundles a second high-level memory-wording signal.
- Remaining: Issue #3 and Issue #4, clarify `recurring-failures` as anti-trap memory rather than bug tracking. These remain duplicates in substance and should execute as one bounded memory-contract slice, not as parallel issue threads.
...
- Recurring-failures anti-trap contract: clarify `memory/mistakes/recurring-failures.md` as anti-trap memory for repeated failure patterns and verified lessons, not as a general bug tracker or compound backlog. Source: GitHub issues `#3` and `#4`. Promote when another maintenance pass still finds duplicate or malformed recurring-failures intake after one cleanup cycle.
- Memory high-level framing hardening: tighten the top-level memory-package wording so skimming readers are pushed toward anti-rediscovery value, one-home ownership, and subordination to planning and canonical docs earlier. Source: GitHub issue `#5` compound intake tail. Promote when the memory package docs or workflow framing are next revised.
