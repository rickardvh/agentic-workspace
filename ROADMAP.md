...
| 4 | `Mature-repo setup no-op detection` | GitHub issue `#72` | On a mature already-customized repo, setup currently adds little useful signal but does not clearly say that no new seed surfaces are needed, which weakens the quiet-by-default goal. | Another mature repo shows the same low-value setup result set, or the next setup/product-shape tranche is ready to improve graceful no-op behavior before adding more guidance surfaces. |
