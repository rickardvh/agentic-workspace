...
| Candidate | Source | Why now | Promote when |
| --- | --- | --- | --- |
| `Planning beta surface alignment` | GitHub issue `#26`; `docs/maturity-model.md`; `README.md`; `packages/planning/README.md` | Planning is now claimed as `beta`, but at least one public surface still needs to line up with that claim so maturity signaling stays one-home and trustworthy. | Another public-facing surface disagrees on planning maturity or contract status, or the next maintainer/docs pass touches planning front-door messaging anyway. |
