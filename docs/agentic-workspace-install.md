...
For older adoption records without a structured baseline, an unchanged ledger matching the authenticated, committed installed hash supplies the prior package baseline. This lets refresh retire leaked source-repository policy. A ledger edited since that adoption does not gain inferred package custody; its declarations remain preserved or blocked for reconciliation. The portable read profile retains the installed skill-registry route without the source-maintenance registry reference.
