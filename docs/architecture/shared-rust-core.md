...
The repository's existing source owner configures `[assurance] decision_record_target` as a destination for future decisions. This setting alone does not admit historical decisions: an absent directory or ordinary Markdown ADRs without typed AW records stays quiet during changed-scope work, including package refresh and plain no-retention proposals. Without a revision, bounded confined discovery uses typed records only to detect whether admission is needed; unadmitted repository bytes never contribute authority. When current work depends on repository-source admission and no already-admitted fallback safely preserves the decision, relevant records (or malformed typed material whose relevance cannot be established) fail closed with the missing `assurance.decision_record_revision`, source path and required repository/source-owner action. When an admitted Memory fallback already preserves the current decision, its consequence remains available and the preferred repository owner stays pending reconciliation. Failed or unavailable stronger-owner admission does not erase that fallback, and pending reconciliation alone adds no task-wide blocker. Native publications continue through their independently checked bounded-answer custody; they do not create a Git trust pin. An explicitly referenced historical predecessor still requires its admission.

The optional `decision_record_revision` is a full immutable Git commit, explicitly admitting the records and their semantic provenance within that archive. Its existing provenance/currentness checks remain unchanged. It is source admission, not discovery or cryptographic human identity authentication. Do not automatically set it to HEAD or infer it from Git tracking, actor labels, path recognition, or installation. Configuration changes need the repository owner's normal authority/review. Ordinary requests cannot supply or override this admission. Low-level compiler/source-contribution APIs accept independently admitted owner facts; those inputs are not ordinary public intention arguments.
