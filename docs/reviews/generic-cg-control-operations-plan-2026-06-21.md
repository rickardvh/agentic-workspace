...
| `output.emit` view extension | Format dispatch and serialisation using host-provided view specs. | Semantic payload construction and view policy. | Workspace config/defaults/prompt/system-intent output paths. | A synthetic command emits JSON and text from one host-neutral operation. |
...
- AW consumes the primitive only through declared IR metadata and regenerated artefacts.
