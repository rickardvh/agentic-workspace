...
| `eligible-refusal-only` | Destructive command has proved refusal behaviour that may mature separately, while destructive apply dispatch remains deferred. |
| `deferred-destructive` | Command may remove state and needs destructive/refusal proof plus review-required signalling before mutation-capable promotion. |
...
| `install`, `init`, `adopt`, `upgrade` apply behaviour | `deferred-mutation` | hand-authored until mutation promotion gates pass | workspace lifecycle primitives | #660 |
| `uninstall` apply behaviour | `deferred-destructive` | hand-authored until destructive promotion gates pass | workspace lifecycle primitives | #660 |
...
- Black-box process behaviour is checked by `uv run pytest tests/test_generated_tool_conformance.py -q`.
