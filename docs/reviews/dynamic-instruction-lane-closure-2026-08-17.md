...
| Adversarial conformance (#2598) | `tests/fixtures/dynamic_instruction_scenarios.json`, `tests/test_dynamic_instruction_projection.py` | Seven small behaviour-level scenarios cover recall, precision, freshness, causal effect, coherence, Memory/Planning authority, and a focused procedure route. Expectations are hand-maintained effects, not snapshots generated from runtime mappings. |
