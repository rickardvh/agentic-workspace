...
- Harness scoring consumes successful command-output receipts keyed by canonical `operation_id`, status, outcome, transition, mutation effect, and owned artifact kinds. Mentioning or promising an operation does not pass.
...
- Total successful-completion cost includes orchestrator and worker context, commands, transport, review, repair, retry/restart, and proof. The maintained comparison contains both a delegation-beneficial bounded mechanical case and a justified selected-current judgment-heavy case.
