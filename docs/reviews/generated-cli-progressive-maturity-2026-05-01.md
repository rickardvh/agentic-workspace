...
| Root workspace Python CLI | `agentic-workspace config` | handwritten root parser/dispatch | `runtime-backed-read-only-adapter` | `workspace.target-root.resolve`, `workspace.config.load`, `workspace.config.emit` | generated freshness, Python parser/dispatch test, black-box process conformance |
