...
| `runtime-backed-read-only-adapter` | Generated parser/dispatch is the interface authority and delegates to hand-owned runtime primitives. | Python root `defaults` and `config`, Planning `status`, and Memory `status`. |
...
| Root workspace TypeScript package | `agentic-workspace defaults`, `agentic-workspace config` | `runnable-read-only-adapter` | unchanged | canonical Python process handoff | Node-backed conformance |
