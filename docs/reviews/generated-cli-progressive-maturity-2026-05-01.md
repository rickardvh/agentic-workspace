...
| `runtime-backed-read-only-adapter` | Generated parser/dispatch is the interface authority and delegates to hand-owned runtime primitives. | Python root `defaults`, Planning `status`, and Memory `status`. |
...
| Root workspace TypeScript package | `agentic-workspace defaults` | `runnable-read-only-adapter` | unchanged | canonical Python process handoff | existing Node-backed conformance |
...
| --- | --- | --- | --- |
| `modules` | handwritten root parser/dispatch backed by CLI contract manifests | `runtime-backed-read-only-adapter` | Small runtime boundary; good next candidate. |
| `config` | handwritten root parser/dispatch backed by CLI contract manifests | `runtime-backed-read-only-adapter` | Needs config loading error conformance. |
