...
Purpose: classify remaining hand-authored CLI authority before any generated shipped CLI adapter work under #356. This is a review/evidence artefact, not ordinary startup input.

## Summary

The root `agentic-workspace` CLI is the only inspected CLI with contract-backed command and option shape today. Its parser is assembled from `src/agentic_workspace/contracts/cli_commands.json` and shared option groups, and every root command has a draft operation contract. Runtime dispatch is still hand-authored in Python, which is acceptable for primitive behaviour but means no command is yet a generated adapter end to end.
...
| `skills`, `system-intent`, `note-delegation-outcome` | intentionally runtime-owned primitive behaviour | Interface can be generated later, but registry inspection, metadata refresh, and local-only append behaviour should stay runtime primitives. |
| `setup`, `prompt` | still hand-authored interface authority | The command shape is in the manifest, but the behaviour is mostly rendered guidance and should wait until generated-adapter rendering rules are proven on smaller read-only report commands. |
...
| `promote-to-plan`, `archive-plan` | intentionally runtime-owned primitive behaviour | These mutate planning state and encode closeout/cleanup policy. Interface extraction is possible later, but behaviour should stay implementation-owned. |
...
| `sync-memory` | intentionally runtime-owned primitive behaviour | It proposes repository memory changes and improvement candidates; generation should only cover interface wiring after behaviour remains primitive-owned. |
...
- Runtime behaviour still calls the existing defaults primitive path.
