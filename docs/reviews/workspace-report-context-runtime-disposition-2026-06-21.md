...
| `_select_workspace_operation_fields` | `generic-cg-control` | command-generation `payload.project` plus deleted AW symbol | AW owns config payload construction and tiny/compact config view policy; CG owns exact selector projection. | Done for exact selectors: `config.report` loads a host-owned payload, `payload.project` applies `--select`, generated artefacts were refreshed, and the accepted boundary entry was removed. | No remaining hand-owned symbol. Tiny/compact/full config view policy remains in `_load_workspace_operation_config`/`_emit_workspace_operation_output`. |
...
| `_run_start_context_adapter` | `mixed-policy-plus-kernel` | AW semantic primitive | AW owns next-safe-action, issue grounding, memory packet, proof posture, and startup judgement. | Only split after named smaller semantic payload functions exist with parity tests. | Startup is the main AW operating-loop judgement surface. |
