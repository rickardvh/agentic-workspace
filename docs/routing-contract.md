...
- **Primary Command**: `agentic-workspace install --target ./repo --preset full`.
...
3. **Raw Detail**: Open `TODO.md` or execplan prose only when the compact summary is insufficient.
...
- **Findings**: Setup findings are advisory. Promote them to `TODO.md` only when they represent clear, actionable work.
...
- **Promotion**: Move work from `ROADMAP.md` to `TODO.md` when scope and dependencies are clear.
