...
A previous passing result may be reusable when its relevant dependencies are unchanged. It is not fresh evidence for different code merely because it has the same test name. Human review and explicitly required checks cannot be replaced by a favorable summary.
