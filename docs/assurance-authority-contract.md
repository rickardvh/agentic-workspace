...
4. `admit_external_evidence` treats provider output as a candidate until a repository-owned evidence authority admits the exact producer, proof route, evidence class, result contract, allowed result, optional application, and current proof subject.

Evidence authorities live in `.agentic-workspace/verification/manifest.toml` under `[evidence_authorities.<id>]`. They are queryable through the Verification report. The producer and transport are separate identities: delivery through an API, CI bridge, or connector never authorizes the sender. External evidence remains a bounded reference, not an embedded log or durable ledger.
