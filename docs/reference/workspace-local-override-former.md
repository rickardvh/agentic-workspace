...
# Former source recognition: Agentic Workspace Local Mixed-Agent Override

Bounded version-1 source recognition only. Not a current authoring or permission contract. Preserve unresolved source meaning; exact current edits cannot create deprecated values.

- Source schema: `src/agentic_workspace/contracts/schemas/workspace_local_override_former.schema.json`
...
| --- | --- | --- | --- | --- | --- | --- |
| (root) | object | yes |  | Bounded version-1 source recognition only. Not a current authoring or permission contract. Preserve unresolved source meaning; exact current edits cannot create deprecated values. |  | x-agentic-workspace-doc-role: "contract-reference" |
...
| `runtime.cheap_bounded_executor_available` | boolean | no |  | Deprecated compatibility summary scheduled for removal by 1.0.0. Canonical target capability, cost, and transport readiness derive availability. |  |  |
| `handoff` | object | no |  | Local preferences for internal or external delegation handoff. |  |  |
| `handoff.prefer_internal_delegation_when_available` | boolean | no |  | Deprecated compatibility preference scheduled for removal by 1.0.0. Canonical constructible transports own transport ranking. |  |  |
...
| `delegation.mode` | enum `"off"`, `"manual"`, `"suggest"`, `"auto"` | no | `"suggest"` | Deprecated compatibility control scheduled for removal by 1.0.0. Use delegation.transport_authority for execution authority. |  |  |
| `delegation.execution_role` | enum `"ordinary-executor"`, `"orchestrator"`, `"bounded-worker"` | no | `"ordinary-executor"` | Deprecated compatibility role scheduled for removal by 1.0.0. Execution role derives from delegation.assignment_policy. |  |  |
...
| `delegation.selection_objective` | string | no |  | Deprecated compatibility objective scheduled for removal by 1.0.0. Canonical best fit ranks safety, capability, quality, and proof before total successful-completion cost. |  |  |
...
| `delegation.underfit_behavior` | enum `"stay-when-safe"`, `"prepare-manual-escalation"`, `"require-delegation"` | no | `"stay-when-safe"` | Deprecated compatibility behavior scheduled for removal by 1.0.0. It derives from delegation.assignment_policy. |  |  |
...
| `delegation.replacement` | object | no |  | Recognized former exact human replacement answer. Preserve for Assignment disposition; new answers use its current revision-bound decision request, not standing configuration. |  | x-agentic-workspace-source-owner: "assignment" |
...
| `setup` | object | no |  | Recognized former revision-bound setup continuation. Preserve unresolved source authority; new decisions and resume identity belong to the configuration owner, not durable local policy. |  | x-agentic-workspace-source-owner: "workspace-config" |
...
| `delegation_targets.<^.+$>.execution_methods` | array of enum `"internal"`, `"cli"`, `"api"`, `"manual"` | no |  | Deprecated compatibility declaration. Canonical transports carry method-specific readiness and payload together. |  |  |
...
| `delegation_targets.<^.+$>.dispatch_adapter_kind` | enum `"process"`, `"host-native"` | no |  | Configured transport adapter class. Both kinds consume the same canonical assignment packet; host-native identifies a host bridge rather than changing assignment semantics. |  |  |
| `delegation_targets.<^.+$>.dispatch_command` | array of string | no |  | Argument-vector template for the configured adapter. Supported placeholders are {target_root}, {output_schema}, {output_file}, and {model}; the sealed prompt is supplied on stdin. |  |  |
| `delegation_targets.<^.+$>.dispatch_output_mode` | enum `"stdout"`, `"json-file"` | no | `"stdout"` | Where the adapter returns its delegated-return JSON. |  |  |
| `delegation_targets.<^.+$>.dispatch_timeout_seconds` | integer | no | `1800` | Maximum configured adapter execution time in seconds. |  |  |
...
| `delegation_targets.<^.+$>.current_economic_evidence` | object | no |  | Recognized former provider-economic observation. New observations belong to the current Assignment evidence input, not configuration. Existing source bytes remain human-owned; this observation grants no discount, capability or authority by itself. |  | x-agentic-workspace-source-owner: "assignment" |
| `delegation_targets.<^.+$>.current_economic_evidence.status` | enum `"available"`, `"unavailable"`, `"exhausted"`, `"contradictory"`, `"unknown"` | no | `"unknown"` | Whether the current economic observation is usable; non-available states never imply a discount. |  |  |
| `delegation_targets.<^.+$>.current_economic_evidence.marginal_cost` | enum `"near-zero"`, `"lower"`, `"equivalent"`, `"higher"`, `"unknown"` | no | `"unknown"` | Provider-neutral current marginal opportunity-cost class. |  |  |
| `delegation_targets.<^.+$>.current_economic_evidence.resource_domain` | string | no |  | Optional resource-pool identity used to explain shared or separate economics. |  |  |
| `delegation_targets.<^.+$>.current_economic_evidence.source` | string | no |  | Provenance for the current economic observation. |  |  |
| `delegation_targets.<^.+$>.current_economic_evidence.observed_at` | string | no |  | Timestamp when the economic observation was made. |  |  |
| `delegation_targets.<^.+$>.current_economic_evidence.expires_at` | string | no |  | Timestamp after which the observation must not affect ranking. |  |  |
...
| `delegation_targets.<^.+$>.escalation_target` | string | no |  | Ignored compatibility alias. Canonical assignment-policy best-fit ranking owns target selection and escalation. |  |  |
...
| `delegation_targets.<^.+$>.last_evaluation` | string | no |  | Recognized former evaluation residue. Preserve the source for target-evidence disposition; do not author or update this field in configuration. |  | x-agentic-workspace-source-owner: "target-evidence" |
