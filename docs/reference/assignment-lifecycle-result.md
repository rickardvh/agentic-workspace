...
Result emitted by public assignment lifecycle operations, including transition status, mutation effect, local artefacts, and recovery guidance.

- Source schema: `src/tooling/contracts/schemas/assignment_lifecycle_result.schema.json`
...
| --- | --- | --- | --- | --- | --- | --- |
| (root) | object | yes |  | Result emitted by public assignment lifecycle operations, including transition status, mutation effect, local artefacts, and recovery guidance. |  | x-agentic-workspace-doc-role: "contract-reference" |
| `artifact_refs` | array of string | yes |  | Local artefact references written or inspected during the lifecycle transition. |  |  |
