...
- Source schema: `src/core/contracts/schemas/workspace_local_override.schema.json`
...
| `delegation.human_override_policy` | enum `"explicit-only"`, `"allowed-with-recorded-reason"`, `"disallowed"` | no | `"explicit-only"` | Exact source-admitted replacement policy: explicit-only requires the current human answer; allowed-with-recorded-reason additionally requires a nonempty recorded reason; disallowed rejects replacement. Comparison never supplies human admission. |  |  |
