...
| `delegation.execution_role` | enum `"ordinary-executor"`, `"orchestrator"`, `"bounded-worker"` | no | `"ordinary-executor"` | Role of the current runtime in assignment decisions. This is separate from how a selected delegation handoff is executed. |  |  |
...
| `delegation.selection_objective` | string | no |  | Human-readable objective for assignment selection, such as minimizing expected total successful-completion cost while preserving quality and proof. |  |  |
...
| `delegation.underfit_behavior` | enum `"stay-when-safe"`, `"prepare-manual-escalation"`, `"require-delegation"` | no | `"stay-when-safe"` | What the current runtime should do when it appears underfit for the selected task class. |  |  |
| `delegation.down_routing_behavior` | enum `"never"`, `"bounded-mechanical-work"`, `"when-cheaper-safe-target-exists"` | no | `"never"` | Whether a capable orchestrator should route bounded mechanical work to a cheaper safe target. |  |  |
