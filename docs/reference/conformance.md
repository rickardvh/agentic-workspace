...
- Source schema: `src/tooling/contracts/schemas/conformance.schema.json`
...
| --- | --- | --- | --- | --- | --- | --- |
| (root) | object | yes |  | Source-maintenance model only; not native/public command authority. Conformance fixture contract for proving an operation adapter behaves as expected. |  | x-agentic-workspace-doc-role: "contract-reference" |
...
| `migration_status` | const `"source-maintenance-only"` | yes |  | Historical generated-operation fixture; never current native/public command conformance. The public command inventory belongs to source_decision_contract.json native_cli. |  |  |
