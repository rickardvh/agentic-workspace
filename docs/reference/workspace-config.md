...
| `assurance.decision_record_target` | string | no |  | Confined repository-relative directory for durable decision records. One optional trailing slash is accepted and removed for archive identity; absolute paths, backslashes, colons, empty interior segments, dot/traversal segments and .git segments are rejected. Exact source-file and changed-path references do not accept a trailing slash. | `"docs/decisions/"` |  |
