...
| `workflow_obligations.<^.+$>` | ref `#/$defs/workflow_obligation` | no |  | Repo-local workflow obligation that can be surfaced when stage and scope match current work. |  | x-agentic-workspace-doc-role: "maintainer"<br>x-agentic-workspace-unknown-properties: "warn" |
