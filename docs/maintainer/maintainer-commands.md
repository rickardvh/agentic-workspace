...
- `make test`, `make test-workspace`, `make test-memory`, `make test-planning`, and the package `make test` lanes run pytest with xdist (`-n 4` on Windows from the root Makefile, `-n auto` elsewhere) by default; override `PYTEST_PARALLEL_ARGS` when you need a different worker count or a serial run.
