...
Operational contracts own canonical behaviour examples. Python owns one authoritative runner. Generated CLIs, future MCP targets, and other transports provide thin adapters that execute the same cases and normalise results.

Do not preserve the current test layout by renaming ordinary tests into contract-shaped files. Convert behaviour only when the primitive, fragment, operation, or command boundary is stable enough that the case will not encode a temporary implementation seam.

## Readiness Gates

Start broad conversion only when all gates are true:

- Portable primitive migration is stable enough that primitive behaviour belongs in command-generation-owned contracts.
- Fragment or composite operation structure is stable enough that reusable workflow behaviour can attach below command-level black-box tests.
- Generated Python and TypeScript targets are fresh and covered by shared conformance runner proof.
- A converted case can name one behaviour owner: primitive, fragment/subflow, operation, command adapter, runner internals, or host-specific orchestration.

If a behaviour cannot name one owner yet, keep the ordinary test and mark the blocker in the inventory.
...
| `tests/test_command_generation_integration.py` | AW integration checks for command-generation dependency wiring and operation fragment support | Keep only AW integration tests that prove dependency wiring; portable primitive behaviour belongs in command-generation contract cases | command-generation plus AW integration |
| `tests/test_workspace_cli_blackbox.py` | Console-script and adapter behaviour | Keep minimal representative adapter tests; convert stable command outputs to operation cases | AW |
| `tests/test_workspace_cli.py` | Mixed CLI compatibility, selectors, and orchestration behaviour | Split: convert stable command outputs; keep selector/orchestration/error-route tests ordinary | AW |
| `tests/test_workspace_config_cli.py`, `tests/test_workspace_defaults_cli.py` | Command behaviour and output shaping | Convert stable output examples to operation cases; keep policy edge cases ordinary until fragments own them | AW |
...
| Surface | Current role | Replacement category | Owner |
| --- | --- | --- | --- |
| `tests/test_public_api.py` | Package API, generated package behaviour, conformance runner drift examples, registry checks | Keep API/schema/generator mechanics ordinary; convert generated command behaviour examples to package-owned contract cases | command-generation |
| `tests/test_primitive_executor.py` | Portable primitive behaviour, fragments, primitive errors, operation dataflow | Convert stable primitive and fragment behaviour to primitive/composite contract cases; keep executor internals and narrow bug repros ordinary | command-generation |
...
1. Stabilise owner boundaries.

   Confirm which primitive and fragment behaviours live in command-generation versus AW. Do this before moving tests so converted cases do not preserve pre-migration runtime seams.

2. Convert portable primitive behaviour in command-generation.

   Move stable examples for filesystem, JSON, TOML, payload assembly, payload verification, output emit, fragment execution, and operation dataflow into contract-owned cases. Keep executor error mechanics ordinary where they are testing runner implementation rather than public behaviour.
...
4. Collapse duplicated ordinary tests.
   ...
   For each converted behaviour, delete or merge the old regression only after the contract case proves the same behaviour through the shared runner. Leave one representative adapter/black-box test per transport risk.
...
- adapter argument mapping, result extraction, exit/error normalisation, and capability reporting;
