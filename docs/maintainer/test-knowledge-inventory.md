...
| --- | --- | --- | --- |
| `tests/test_workspace_report_cli.py` | Report and closeout rendering accumulated many one-off regressions around selector, trust, caveat, and projection shapes. | Compact migration record in this table; active proof remains in smaller report/summary/CLI surfaces and future contract-owned cases. | The legacy source file was deleted. New report behavior should be proven by contract or focused scenario tests. |
| `tests/test_model_cli_harness.py` | Dogfooding scorer and adapter incidents accumulated as many narrow fixture-specific checks. | Compact migration record in this table; ownership remains with the model CLI harness surfaces and future focused harness tests. | The legacy source file was deleted so the evaluator harness cannot dominate ordinary suite count. |
| `tests/test_workspace_start_preflight_cli.py` | Startup and preflight routing behavior accumulated many branch-shape regressions. | Compact migration record in this table; future proof should use small startup matrices or AW contract checks. | The legacy source file was deleted; permanent proof should be narrower and owner-driven. |
| `packages/planning/tests/test_summary.py` | Planning summary tests accumulated many projection and historical-state variants. | Compact migration record in this table; active planning proof remains in focused package tests and planning surface checks. | The legacy source file was deleted while package-local executable proof stays closer to current contracts. |
| `tests/test_contract_tooling.py` | Contract tooling tests accumulated generated-adapter and schema-history assertions. | Compact migration record in this table; active proof remains in generated package checks, maintainer surfaces, and focused conformance tests. | Stable generated behavior should move to conformance/checker ownership instead of one large ordinary regression file. |
| `tests/test_workspace_lifecycle_cli.py` | Lifecycle command behavior accumulated broad workflow regressions. | Compact migration record in this table; active proof remains in package install/lifecycle checks and small root smoke paths. | The legacy source file was deleted while the executable suite keeps only current high-value proof. |
