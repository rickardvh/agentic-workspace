...
| --- | --- | --- | --- |
| `tests/test_workspace_report_cli.py` | Report and closeout rendering accumulated many one-off regressions around selector, trust, caveat, and projection shapes. | Archived source: `docs/maintainer/retired-test-knowledge/test_workspace_report_cli.py`; active proof remains in smaller report/summary/CLI surfaces and future contract-owned cases. | The file is now maintainer knowledge, not a permanent executable proof bucket. New report behavior should be proven by contract or focused scenario tests. |
| `tests/test_model_cli_harness.py` | Dogfooding scorer and adapter incidents accumulated as many narrow fixture-specific checks. | Archived source: `docs/maintainer/retired-test-knowledge/test_model_cli_harness.py`; ownership remains with the model CLI harness surfaces and future focused harness tests. | The archived file preserves scoring knowledge while preventing the evaluator harness from dominating ordinary suite count. |
| `tests/test_workspace_start_preflight_cli.py` | Startup and preflight routing behavior accumulated many branch-shape regressions. | Archived source: `docs/maintainer/retired-test-knowledge/test_workspace_start_preflight_cli.py`; future proof should use small startup matrices or AW contract checks. | The file records prior startup/preflight knowledge, but permanent proof should be narrower and owner-driven. |
| `packages/planning/tests/test_summary.py` | Planning summary tests accumulated many projection and historical-state variants. | Archived source: `docs/maintainer/retired-test-knowledge/planning_test_summary.py`; active planning proof remains in focused package tests and planning surface checks. | Historical projection variants are preserved for reference while package-local executable proof stays closer to current contracts. |
| `tests/test_contract_tooling.py` | Contract tooling tests accumulated generated-adapter and schema-history assertions. | Archived source: `docs/maintainer/retired-test-knowledge/test_contract_tooling.py`; active proof remains in generated package checks, maintainer surfaces, and focused conformance tests. | Stable generated behavior should move to conformance/checker ownership instead of one large ordinary regression file. |
| `tests/test_workspace_lifecycle_cli.py` | Lifecycle command behavior accumulated broad workflow regressions. | Archived source: `docs/maintainer/retired-test-knowledge/test_workspace_lifecycle_cli.py`; active proof remains in package install/lifecycle checks and small root smoke paths. | The archive keeps historical lifecycle knowledge while the executable suite keeps only current high-value proof. |
