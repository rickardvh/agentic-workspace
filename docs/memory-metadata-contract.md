...
Every memory note MUST be declared in `memory/manifest.toml`.
...
This uses the manifest and `memory/index.md` to find the smallest set of relevant notes.
...
- **Weak Authority for Current**: Notes in `memory/current/` are for orientation and continuation, not durable facts.
