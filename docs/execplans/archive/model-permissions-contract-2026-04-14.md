...
## Context
See GitHub issue `#76`. The local override contract (`agentic-workspace.local.toml`) is extremely useful for defining capability constraints (e.g., `strong_planner_available = true`). However, there's no explicitly standardized way to bound "blast radius" or execution safety (autonomous run permissions).
...
## Validation
1. Create a `agentic-workspace.local.toml` with `[safety]` constraints.
