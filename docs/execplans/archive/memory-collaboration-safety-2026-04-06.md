...
- Strengthen the installed Agentic Memory contract for larger git-heavy teams by making `memory/current/` explicitly weak-authority, easier to compress or replace, and less likely to duplicate durable memory notes under concurrent edits.
...
- Durable facts should stay in canonical memory notes or checked-in docs rather than migrating into `memory/current/`.
...
- The shipped memory manifest no longer presents `memory/current/` notes as canonical durable authority.
