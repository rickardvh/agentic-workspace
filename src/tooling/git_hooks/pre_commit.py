...
POST_FORMAT_COMMANDS = (
    ...
    [sys.executable, "src/tooling/check/check_no_absolute_paths.py"],
)
...
def _format_candidates() -> list[Path]:
    ...
    return [
        ...
        and (not path.is_relative_to("src/tooling") or path.is_relative_to("src/tooling/python"))
    ]
...
def _validation_environment() -> dict[str, str]:
    ...
    if run_id and environment.get("VALIDATION_JOIN_TOKEN") == f"join:{run_id}":
        ...
        return environment
    result = subprocess.run(
        [sys.executable, "src/tooling/check/allocate_validation_run_id.py"],
        cwd=REPO_ROOT,
        check=True,
        capture_output=True,
        text=True,
