...
def _review_prompt(review: Review, *, branch: str = "") -> str:
    ...
    return (
        ...
        f"Required work:\n{review.findings}\n\n"
        f"{'You are on branch ' + branch + '; push with git push origin ' + branch + '. ' if branch else ''}Address these findings, run the appropriate proof, push a new head, then explicitly record it with `uv run python src/tooling/github/chatgpt_review_loop.py handoff --pr {review.pr} --existing-only`. "
        'After proof and push, record their exact outcomes with `python src/tooling/github/chatgpt_review_loop.py job-result --session-id $CODEX_THREAD_ID --proof-status passed|failed --proof-command "<command>" --proof-exit-code <exit> --push-status passed|failed`. Do not merge from this continuation.'
    )
