...
Trace-required result records must also include the normalised `agentic-workspace/operating-loop-decision/v1` packet. The older `decisions` object remains a compact human-readable trace, while `operating_loop` is the enum-backed evaluation evidence for Memory, Planning, Verification/proof, residue owner, and safe claim state.

For #1680 completion-cost work, each probe can also emit `agentic-workspace/external-agent-completion-cost-observations/v1`. The observation packet is maintainer-evaluation evidence only, and records compact behaviour-cost signals such as AW command count, proof command count, reread events, proof churn, over-planning, review/repair loops, handoff recovery status, unsafe closure claims, used AW output sections, and classified cost drivers.

Configured-orchestration behavioural scenarios require command-output operation receipts. A final response that names `assignment.export`, promises to use it, or recommends a route does not pass. Receipt requirements may constrain canonical result kind, operation id, transition, status, outcome, mutation effect, nested fields, and owned artefact kinds while leaving runtime semantics with the assignment operation owner.
...
- `scenario-probes.json`: canonical probes and artefact-backed host-install probe definitions.
...
- `provider-availability-2026-08-24.json`: current CLI discovery and explicit live-proof gaps; availability alone is not a behavioural pass.
...
- artefact-backed host operation has a defined probe;
...
- completion-cost observations can distinguish static/schema suspicion from behaviour-cost evidence.

Historical orchestration and context-cost observations describe the former Python host. Their retired capture programs are available in Git history; these records do not establish support or current native execution evidence.
