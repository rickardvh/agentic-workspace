...
pub(crate) fn enclave() -> Value {
    serde_json::from_str(include_str!("contracts/enclave.json")).expect("Memory enclave contract")
}
...
pub(crate) fn validate_manifest(manifest: &Value) -> Result<(), CoreError> {
    let schema: Value = serde_json::from_str(include_str!("contracts/manifest.schema.json"))
