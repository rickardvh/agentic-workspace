...
def _emit_report_text(payload: dict[str, Any]) -> None:
    ...
    module_enablement = payload.get("feature_tier", {})
    if isinstance(module_enablement, dict):
        active_modules = module_enablement.get("active", {})
        if isinstance(active_modules, dict) and active_modules.get("id"):
            modules = ", ".join(active_modules.get("modules", [])) or "(none)"
            print(f"Module enablement: {modules}")
