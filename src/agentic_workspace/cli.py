...
class WorkspaceArgumentParser(argparse.ArgumentParser):
    ...
    def error(self, message: str) -> NoReturn:
        if "invalid choice" in message and "command" in message:
            ...
            message = (
                f"{message}\nStartup tip: run 'agentic-workspace start --task \"<task>\" --format json' for normal startup "
...
def _diagnostic_profile(args: argparse.Namespace, *, default: str) -> str:
    return "full" if getattr(args, "verbose", False) else str(getattr(args, "profile", default))
    ...
    *,
    ...
    if not active_planning_present and task_mentioned_paths and not changed_paths and not _is_config_posture_task(task_text):
        ...
        payload["immediate_next_allowed_action"] = {
            ...
            "summary": (
                "The task text names existing repo paths. Run the implement surface for those paths before broader startup "
...
def _selector_first_start_payload(payload: dict[str, Any], *, cli_invoke: str) -> dict[str, Any]:
    ...
    selected: dict[str, Any] = {
        ...
        "drill_down": {
            ...
            "examples": [
                _command_with_cli_invoke(
                    command="agentic-workspace start --select cli_invocation,durable_intent --format json",
                    cli_invoke=cli_invoke,
                ),
                _command_with_cli_invoke(
