...
def workspace_pointer_block(*, cli_invoke: str = DEFAULT_CLI_INVOKE) -> str:
    return (
        ...
        "4. Execute only the exact returned `primary_action` through `invoke --input <action.json>` with the same context and `--format json`; use the current returned continuation, or its exact re-entry when unavailable. Never retry an effect merely because continuation failed. Never manufacture an invocation, omit a blocker or use the former Python host to bypass an unavailable native owner. Direct work stays direct when the current contract permits it.\n"
